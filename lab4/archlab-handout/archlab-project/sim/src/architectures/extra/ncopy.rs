...
//! 因此可以设置一个参数保存之前产生的cc, 从而当cond要更新时可以直接取之前产生的cc更新, 而不需要等待reg_cc, 即从串联变为并联
...
crate::define_stages! {
    ...
    ExecuteStage e {
        ...
        prev_cc: ConditionCode = CC_INIT //上次产生的cc
    }
    /// Memory Access Stage
    MemoryStage m {
...
u8 d_ifun = D.ifun;
Stat d_stat = D.stat;
ConditionCode tpcc = reg_cc.cc; // 保存产生的cc
